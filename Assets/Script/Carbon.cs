...
public class Carbon : MonoBehaviour
{
    ...
    {
        UpdateHealthBar();
        currentHealth = (Resource.Instance.carbon) / 100f;
        if (currentHealth <= 0)
        {
            Ending.SetActive(true);
            if (Resource.Instance.money >= Resource.Instance.prestige &&
                Resource.Instance.money >= Resource.Instance.technology)
            {
                endtext.text=("你成功了。市民们在你的带领下实现了完美的无碳生活。再也不会有一丝一毫的二氧化碳及其他温室气体被肆意排放到空中了。人们总是打量着自己的富足生活感叹：难以想象不久之前我们还要通过扩建污染性强的制造业工厂去获取微博的薪水。哈哈，现在，我们从别的国家买就好了。");
            }
            if (Resource.Instance.technology >= Resource.Instance.money &&
                Resource.Instance.technology >= Resource.Instance.prestige)
            {
        ...
        if (currentHealth >= 1)
        {
            ...
            if (Resource.Instance.money >= Resource.Instance.prestige &&
                Resource.Instance.money >= Resource.Instance.technology)
            {
                endtext.text=("你一败涂地。在花费了大量精力，科技，和权威尝试阻止事态崩溃后，天上的星星，月亮，和太阳还是都消失了。城市被笼罩上了一层永远驱逐不走的雾霾屏障，同时也熄灭了名为希望的火苗。但没关系——你有钱啊！权贵们已经从中赚取了数不尽的财富，跑到随便一个北欧国家，太平洋小岛去享受人生也是一种乐趣啊。");
            }
            if (Resource.Instance.technology >= Resource.Instance.money &&
                Resource.Instance.technology >= Resource.Instance.prestige)
            {
